...
import type { MatchMode, Move, PlayerId } from "../engine/types";
import {
  applyPlayerMove,
  createHostRoom,
  deriveLobby,
...
import type { ClientMessage, HostMessage, LobbySnapshot, PlayerView } from "./protocol";

/**
 * PeerJS(WebRTC) 기반 멀티플레이 세션.
 * - 별도 백엔드/계정이 전혀 필요 없다: 연결 중개는 PeerJS 공용 무료 서버,
 *   게임 데이터는 브라우저끼리 P2P로 직접 주고받는다.
 * - 방장 브라우저가 딜러(단일 진실 공급원) 역할을 한다. 참가자는 자기
 *   손패와 공개 정보만 받는다.
 * - 한계: 방장이 창을 닫으면 방이 사라진다 (캐주얼 게임 전제로 허용).
 */

const PEER_ID_PREFIX = "mse-domino-";
const ROOM_CODE_CHARS = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
const PLAYER_ID_KEY = "mini-game-platform:domino:player-id";
...
export function getMyPlayerId(): PlayerId {
...
export async function hostRoom(
  ...
  let room: HostRoom = createHostRoom(roomCode, myPlayerId, nickname, mode, targetScore);
  const connections = new Map<DataConnection, PlayerId | null>();

  function send(conn: DataConnection, message: HostMessage) {
    conn.send(message);
  }
  ...
  function broadcast() {
    if (!room.match) {
      ...
      for (const [conn, playerId] of connections) {
        if (playerId) send(conn, { t: "lobby", lobby });
      }
      return;
    }
  ...
  peer.on("connection", (conn) => {
    ...
    conn.on("data", (raw) => {
      ...
      if (msg.t === "join") {
        ...
        if (!result.ok) {
          send(conn, { t: "reject", reason: result.reason });
          return;
        }
        room = result.room;
...
export async function joinRoomByCode(
  ...
  conn.on("open", () => {
    const join: ClientMessage = { t: "join", playerId: myPlayerId, nickname };
    conn.send(join);
  });
  ...
  return {
    ...
    nextRound: () => {},
    play: (move) => {
      const msg: ClientMessage = { t: "play", playerId: myPlayerId, move };
      conn.send(msg);
    },
    close: () => {
      peer.destroy();
